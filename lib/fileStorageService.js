...
const codebedRegex = /^codebed\s+([\w]+)$/;
md.use(mdContainer, 'codebed', {
    validate: function (params) {
        return params.trim().match(codebedRegex);
    },
    render: function (tokens, idx) {
        const m = tokens[idx].info.trim().match(codebedRegex);

        if (tokens[idx].nesting === 1) {
            // opening tag
            const codepenSlug = md.utils.escapeHtml(m[1]);
            return `<code-bed data-slug-hash="${codepenSlug}"></code-bed>\n`;
        }
