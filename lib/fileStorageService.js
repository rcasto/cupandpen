...
async function getPublishedContentMetadata() {
    console.log('Reading all content metadata from file system');
...
async function getPublishedContentData(name) {
    console.log(`Reading ${name} from file system`);
...
async function getContent(name) {
    ...
    if (!contentMetadataMap[name]) {
        throw new Error(`${name} is not the title of a valid content piece`);
    }
